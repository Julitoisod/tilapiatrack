                ...
                Forms\Components\Select::make('feeding_program_id')
                    ->options(function () {
                        return \App\Models\FeedingProgram::with('feed')
                            ...
                            ->mapWithKeys(function ($program) {
                                ...
                                return [
                                    $program->id => $program->name . ' - ' . $program->feed->name,
                                ];
                            });
                    })->label('Feeding Program | Feeds ')
                    ->searchable()
                    ->preload()
                    ->required()
                    ->reactive()
                    ->afterStateUpdated(function ($state, callable $set) {
                        ...
                        if ($program) {
                            ...
                            $set('end_date', $startDate->addWeeks(intval($program->age_range))->toDateString());
                        }
                    }),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('feedingProgram.name')
                    ->numeric()
                    ->label('Feedinng Program')
                    ->sortable(),
                Tables\Columns\TextColumn::make('feedingProgram.fingerling_age_range')
                    ->label('Fingerling Age')
                    ->formatStateUsing(fn($state) => $state . ' weeks'),
                Tables\Columns\TextColumn::make('feedingProgram.juvenile_age_range')
                    ->label('Juvenile Age')
                    ->formatStateUsing(fn($state) => $state . ' weeks'),
                Tables\Columns\TextColumn::make('feedingProgram.subadult_age_range')
                    ->label('Sub-Adult Age')
                    ->formatStateUsing(fn($state) => $state . ' weeks'),
                Tables\Columns\TextColumn::make('feedingProgram.adult_age_range')
                    ->label('Adult Age')
                    ->formatStateUsing(fn($state) => $state . ' weeks'),
                Tables\Columns\TextColumn::make('feedingProgram.fingerling_protein_content')
                    ->label('F.Protein')
                    ->formatStateUsing(fn($state) => $state . '%'),
                Tables\Columns\TextColumn::make('feedingProgram.adult_protein_content')
                    ->label('A.Protein')
                    ->formatStateUsing(fn($state) => $state . '%'),
                ...
                //     ->label('Fish Size')
                //     ->sortable(),
                // Tables\Columns\TextColumn::make('feedingProgram.age_range')
                //     ->label('Age Duration')
                //     ->formatStateUsing(fn($state) => $state . ' weeks')
                //     ->searchable(),
                // Tables\Columns\TextColumn::make('feedingProgram.fish_amount')
                //     ->label('Feed Amount')
                //     ->formatStateUsing(fn($state) => $state . ' %')
                //     ->searchable(),
                // Tables\Columns\TextColumn::make('feedingProgram.protein_content')
